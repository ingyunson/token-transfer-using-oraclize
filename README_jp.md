...
#### ファイルの名： `ERC20.sol`と` token_youtube_viewcount.sol`

etherではなく、個別に発行したトークンを送信させるコントラクトです。 `ERC20.sol`を利用して、トークンを作成し、このトークンのコントラクトアドレスを利用して、` token_youtube_viewcount.sol`を介してトークンをコントロールします。
...
------------
#### ファイルの名： `token_youtube_count.sol`
...
#### ファイルの名： `token_timer_youtube_count.sol`
